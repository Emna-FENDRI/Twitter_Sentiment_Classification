...
def load_data(path_train_pos = PATH_TRAIN_POS , path_train_neg = PATH_TRAIN_NEG, path_train_pos_full = PATH_TRAIN_POS_FULL, path_train_neg_full = PATH_TRAIN_NEG_FULL, full = False):
    if(full):
        pos_train = pd.read_csv(path_train_pos_full, sep = '\r',  names = ['Text'])
        pos_train.insert(1, 'Target', 1)
        neg_train = pd.read_csv(path_train_neg_full, sep = '\r',  names = ['Text'])
